...
import { useCreateConnectLink } from "@/lib/corsair-client";
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f7f7f4] text-neutral-950 transition-colors duration-500 dark:bg-neutral-950 dark:text-white">
      ...
      <main className="relative z-10 min-h-[90vh]">
        ...
        <section className="relative z-20 mx-auto flex max-w-7xl flex-col items-center px-4 pt-24 pb-16 text-center sm:px-6 sm:pt-32 lg:px-8">
          <motion.h1
...
function GithubConnectButton() {
  ...
  const handleClick = async () => {
    const link = await mutate({
      plugin: "github",
      tenantId: "bxOmEKg79xNpXuEJTWVzqEi4kRobAznZ",
    });
    if (link) window.open(link.connectUrl, "_blank");
  };
  ...
  return (
    ...
    >
      {loading ? (
        <>
          <div className="mr-2 size-4 animate-spin rounded-full border-2 border-neutral-950/30 border-t-neutral-950 dark:border-white/30 dark:border-t-white" />
          Connecting...
        </>
      ) : (
        <>
          <svg className="mr-2 size-5 fill-current" viewBox="0 0 24 24">
