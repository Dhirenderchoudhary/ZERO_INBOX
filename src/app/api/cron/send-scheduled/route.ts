...
export async function GET(req: NextRequest) {
  ...
  const due = await db.select().from(scheduledEmails).where(
    and(eq(scheduledEmails.sent, false), lte(scheduledEmails.sendAt, new Date()))
  );

  let sent = 0;
  for (const email of due) {
    try {
      const raw = encodeRawEmail({ to: email.to, subject: email.subject, body: email.body, cc: email.cc ?? undefined });
