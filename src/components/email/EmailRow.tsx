...
export function EmailRow({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`group relative flex cursor-pointer items-start gap-3 px-4 py-3.5 transition-all duration-200 select-none ${TRIAGE_CLASS[priority] ?? "triage-none"} `}
      style={{
    ...
    >
      ...
      <div className="relative z-10 mt-[6px] flex-shrink-0">
        <span
          className="block h-2 w-2 rounded-full transition-opacity duration-300"
          style={{
      ...
      {email.isStarred && (
        <Star
          size={14}
          className="relative z-10 mt-[4px] flex-shrink-0"
          fill="var(--accent)"
          color="var(--accent)"
