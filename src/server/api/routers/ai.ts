...
Respond with ONLY the category word. Nothing else.`
      ...
      const valid = ['urgent', 'needs_reply', 'fyi', 'newsletter', 'other'] as const;
      const priority = valid.includes(raw as any) ? raw as typeof valid[number] : 'other';
        ...
        try {
          const msg = await mistral.chat.complete({
            ...
            messages: [
              {
                role: 'system',
                content: 'Classify email: urgent, needs_reply, fyi, newsletter, or other. Reply with ONLY the word.'
              },
              {
                role: 'user',
          ...
          const content = msg.choices?.[0]?.message?.content;
          const rawText = (typeof content === 'string' ? content : '').trim().toLowerCase();
    ...
    .mutation(async ({ input, ctx }) => {
      const msg = await mistral.chat.complete({
        ...
        messages: [
          {
            role: 'system',
            content: 'Summarize this email in 1-2 sentences. Be direct and specific. No fluff.'
          },
          {
            role: 'user',
    ...
    .mutation(async ({ input, ctx }) => {
      const msg = await mistral.chat.complete({
        ...
        messages: [
          {
            role: 'system',
            content: 'Draft a brief, professional email reply. Just the body text — no subject, no greeting needed. Keep it concise.'
          },
          {
            role: 'user',
      ...
      for (const action of parsed.actions ?? []) {
        try {
          if (action.type === 'send_email') {
            ...
            actionsExecuted.push(`Sent email to ${action.to} — "${action.subject}"`);
          }
          ...
          if (action.type === 'create_event') {
            await tenant.googlecalendar.api.events.create({
              ...
              event: {
                ...
                attendees: (action.attendees ?? []).map((e: string) => ({ email: e })),
              }
            });
            actionsExecuted.push(`Created "${action.summary}" with ${action.attendees?.length ?? 0} attendee(s)`);
          }
        } catch (e) {
          actionsExecuted.push(`⚠ Failed: ${action.type} — ${(e as Error).message}`);
        }
      }
